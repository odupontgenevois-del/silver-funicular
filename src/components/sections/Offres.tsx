      ...
      <div className="relative mx-auto max-w-6xl px-6">
        <div className="max-w-2xl mb-16">
          <div className="inline-flex items-center gap-2 mb-5">
            ...
            <span className="text-sm font-medium tracking-[0.2em] uppercase text-[#C4A46B]">Modalités d'intervention</span>
          </div>
        ...
        <div className="glass-copper rounded-xl px-6 py-4 mb-10 max-w-3xl">
          <p className="text-lg text-[#9AABB8] leading-[1.8]">
              ...
              <div className="p-7 flex-1 flex flex-col">
                {o.featured ? (
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#C4A46B] self-start mb-4">
                    ...
                    <span className="text-lg font-bold text-[#0C1B2E] tracking-wide uppercase">{o.label}</span>
                  </div>
                ) : (
                  <div className="text-sm font-medium tracking-[0.16em] uppercase text-[#C4A46B]/60 mb-4">{o.label}</div>
                )}

                <h3 className={`font-display text-2xl font-light mb-2 ${o.featured ? "text-[#EDE9E3]" : "text-[#EDE9E3]"}`}>{o.titre}</h3>
                <p className="text-lg text-[#9AABB8] leading-[1.65] mb-6">{o.sub}</p>

                <ul className="flex flex-col gap-2.5 flex-1 mb-7">
                  {o.items.map((item) => (
                    <li key={item} className="flex items-start gap-2.5 text-lg text-[#9AABB8] leading-[1.5]">
                ...
                <a href="#contact"
                  className={`flex items-center justify-center gap-2 py-3 rounded-xl text-lg font-semibold tracking-wide transition-all duration-200 cursor-pointer ${
                    o.featured
        ...
        <p className="text-sm text-[#9AABB8]/60 text-center mt-6">
          Tous les montants sont définis sur devis après diagnostic. Votre contexte détermine la formule et son dimensionnement.
        </p>

        {/* Scarcity */}
        <div className="mt-6 glass-copper rounded-2xl px-7 py-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5">
          <div>
            <p className="text-lg font-semibold text-[#EDE9E3] mb-0.5">2 créneaux disponibles pour juillet 2026.</p>
            <p className="text-lg text-[#9AABB8]">Les missions sont limitées pour garantir la qualité d'engagement sur chaque dossier.</p>
          </div>
          <a href="#contact"
            className="flex-shrink-0 inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[#C4A46B] text-[#0C1B2E] text-lg font-semibold tracking-wide hover:bg-[#D4B47B] transition-colors cursor-pointer whitespace-nowrap">

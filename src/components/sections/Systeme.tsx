      ...
      <div className="mx-auto max-w-6xl px-6">
        <div className="max-w-2xl mb-16">
          <div className="inline-flex items-center gap-2 mb-5">
            ...
            <span className="text-sm font-medium tracking-[0.2em] uppercase text-[#C4A46B]">Comment ça se passe</span>
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {steps.map((s, i) => (
            <div key={s.n}
              className="relative glass rounded-2xl p-7 hover:bg-white/6 hover:-translate-y-1 transition-all duration-300 group">
              {/* Connector */}
              {i < steps.length - 1 && (
              ...
              <h3 className="text-lg font-semibold text-[#EDE9E3] mb-2">{s.titre}</h3>
              <p className="text-lg text-[#9AABB8] leading-[1.7] mb-4">{s.desc}</p>
              <div className="glass-copper rounded-lg px-3 py-1.5 inline-block">
                <span className="text-lg text-[#C4A46B] font-medium">{s.detail}</span>
              </div>
            </div>
          ))}
        </div>
